...
c-------------------------------------------------------	
	...
	integer, parameter :: nzmax = 450
	integer, parameter :: ntmax = 100000
	real time(ntmax),z(nzmax),p(nzmax),dz(nzmax)
        real f(nzmax*ntmax), f1(nzmax*ntmax), parms(ntmax*nzmax)
...
c Check to make sure the arrays are large enough to read this data
 ...
 928	   format('ntime = ',i6,' ntmax = ', i6,
     &            ' nzm = ',i5,' nzmax = ',i5)
	   ...
	   STOP 'too large arrays in stat2nc.f'
	end if
	...
	err = NF_CREATE(filename2, 
     &                  IOR(NF_CLOBBER,NF_64BIT_OFFSET), 
     &                  ncid)

           if(err.ne.0) then
	      print*,'error when creating ',TRIM(filename2)
...
c--------------------------------------------------------
     ...
     &                       NF_FLOAT,1,-1.)
        ...
        err = NF_PUT_VAR_REAL(ncid, varid, tmp)
           if(err.ne.0) then
	      print*,'error when writing ',TRIM(abbr_name) 
	      print*,nf_strerror(err)
	      STOP 'here3'
	   end if
        print*,long_name
...
c--------------------------------------------------------------
     ...
     &                                  ndimids,vdimids,varid)
           if(err.ne.0) then
	      print*,'error when defining ',TRIM(name) 
	      print*,nf_strerror(err)
	      STOP 'here4'
	   end if
           err = NF_PUT_ATT_TEXT(ncid,varid,'long_name',
     ...
     &                              1,-9999.)
           ...
           if(err.ne.0) then
	      print*,'error when writing ',TRIM(name) 
	      print*,nf_strerror(err)
	      STOP 'here5'
	   end if
